...
def process_dat_files(directory):
    ...
    with tqdm(total=total_files, desc="Processant fitxers", ncols=80, colour="green") as pbar:
        for file in files:
            ...
            try:
                ...
                for line in lines[2:]:
                    ...
                    year = int(values[1])
                    month = int(values[2])
                    daily_values = values[3:]

                    valid_values = [float(v) for v in daily_values if v != "-999"]
                    ...
                    total_negative_999 += daily_values.count("-999")
                    total_other_numbers += len(valid_values)
    ...
    with tqdm(total=len(yearly_data), desc="Processant estadístiques", ncols=80, colour="green") as pbar:
    ...
    print(
        f"{Fore.YELLOW}Any amb la mitjana mensual més alta:{Style.RESET_ALL} {max_avg_year} ({yearly_averages[max_avg_year]:.2f} l/m²)")
